        ...
        <section className='section__cards-results'>
          ...
          <ul className="ul__accommodation-list">
            {productsCities?.map((city) => {
              return <li key={city?.id}><Card id={city?.id} title={city?.name} src={city?.category.urlImagen} address={city?.address} description={city?.desc} category={city?.category.title} punctuation={city?.punctuation} stars={city?.stars} score={city?.score} city={city?.city.cityName + ". " + city?.city.country} latitude={city?.y} longitude={city?.x}  /></li>;
            })}
          </ul>
        </section>
    </>
